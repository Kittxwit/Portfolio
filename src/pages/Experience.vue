    ...
    <main class="pt-32">
      <div class="mx-auto max-w-3xl px-6 py-12">
        ...
        <a
          ...
          class="mb-8 back-btn inline-flex items-center gap-3 text-sm font-medium transition-all duration-300"
        ...
        <!-- Experience Details -->
        <div v-else-if="experience" class="space-y-8">
          <div>
            <h1 class="mb-4 text-4xl font-bold md:text-5xl">{{ experience.role }}</h1>
            <p class="text-lg leading-relaxed" :style="{ color: 'var(--text-secondary)' }">{{ experience.company }} — {{ experience.period }}</p>
          </div>
          ...
          <div>
            ...
            >
              <img
                v-if="experienceImages.length > 0"
                :src="getImageUrl(experienceImages[0])"
                :alt="experience.role"
                class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
              />
              <span v-else class="text-6xl font-bold" :style="{ color: 'var(--accent)' }">{{ experience.role.charAt(0) }}</span>

              <div v-if="experienceImages.length > 0" class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            ...
            <div v-if="experienceImages.length > 1" class="mb-6 grid grid-cols-4 gap-3">
              <div
                v-for="(img, idx) in experienceImages"
                :key="idx"
      ...
      >
        ...
        <!-- Navigation Arrows -->
        <button
          v-if="experienceImages.length > 1"
          @click="prevImage"
        ...
        <div class="relative max-h-[70vh] max-w-[90vw] overflow-hidden rounded-xl">
          <img
            :src="getImageUrl(experienceImages[currentImageIndex])"
            :alt="experience.role"
        ...
        <div class="mt-6 text-center text-white">
          <p class="text-xl font-bold">{{ experience.role }}</p>
          <p class="text-sm text-gray-400 mt-1">{{ t.imageOf.replace('{current}', currentImageIndex + 1).replace('{total}', experienceImages.length) }}</p>
        </div>

        <!-- Thumbnail slide bar in lightbox -->
        <div v-if="experienceImages.length > 1" class="mt-6 flex gap-3 overflow-x-auto max-w-[80vw] px-4 pb-2">
          <button
            v-for="(img, idx) in experienceImages"
            :key="idx"
...
export default {
  ...
  computed: {
    ...
    formattedDescription() {
      if (!this.experience || !this.experience.details) return [];
      return this.experience.details
        .split('\n')
        .map(line => line.trim())
    ...
    experienceImages() {
      ...
      if (this.experience.images && Array.isArray(this.experience.images)) return this.experience.images;
      return this.experience.image ? [this.experience.image] : [];
    },
  },
  ...
  methods: {
    ...
    prevImage() {
      if (this.experienceImages.length === 0) return;
      this.currentImageIndex = (this.currentImageIndex - 1 + this.experienceImages.length) % this.experienceImages.length;
    },
    nextImage() {
      if (this.experienceImages.length === 0) return;
      this.currentImageIndex = (this.currentImageIndex + 1) % this.experienceImages.length;
    },
  },
};
</script>
