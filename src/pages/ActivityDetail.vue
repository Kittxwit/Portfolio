  ...
  <div class="min-h-screen font-sans transition-colors duration-400" :style="{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }">
    ...
    <main class="pt-32">
      <div class="mx-auto max-w-3xl px-6 py-12">
        ...
        <div v-else-if="activityItem" class="space-y-8">
          <div>
            <h1 class="mb-4 text-4xl font-bold md:text-5xl">{{ activityItem.title }}</h1>
            <p class="text-lg leading-relaxed" :style="{ color: 'var(--text-secondary)' }">{{ activityItem.details }}</p>
          </div>

          <div v-if="activityImages.length > 0" class="group relative mb-4 flex h-64 md:h-96 items-center justify-center rounded-2xl overflow-hidden" :style="{ backgroundColor: 'var(--section-alt-bg)', border: '1px solid var(--border-color)' }" @click="openLightbox(currentImageIndex)">
            <img :src="getImageUrl(activityImages[currentImageIndex])" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
          </div>

          <div v-if="activityImages.length > 1" class="mb-6 flex gap-3 overflow-x-auto">
            <div
      ...
      <div v-if="lightboxOpen" class="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/95 backdrop-blur-md" @click.self="closeLightbox">
        ...
        <div class="relative max-h-[70vh] max-w-[90vw] overflow-hidden rounded-xl">
          <img :src="getImageUrl(activityImages[currentImageIndex])" :alt="activityItem.title" class="max-h-[70vh] max-w-[90vw] object-contain" />
        </div>
        <div class="mt-6 text-center text-white">
          <p class="text-xl font-bold">{{ activityItem.title }}</p>
          <p class="text-sm text-gray-400 mt-1">{{ t.imageOf.replace('{current}', currentImageIndex + 1).replace('{total}', activityImages.length) }}</p>
        </div>
        <button
        ...
        <div v-if="activityImages.length > 1" class="mt-6 flex gap-3 overflow-x-auto max-w-[80vw] px-4 pb-2">
          <button v-for="(img, idx) in activityImages" :key="idx" @click="currentImageIndex = idx" class="h-14 w-20 flex-shrink-0 overflow-hidden rounded-lg border-2" :class="currentImageIndex === idx ? 'border-white scale-105 shadow-md' : 'border-transparent opacity-50 hover:opacity-100'">
            <img :src="getImageUrl(img)" class="h-full w-full object-cover" />
          </button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    formattedDescription() { if (!this.activityItem || !this.activityItem.details) return []; return this.activityItem.details.split('\n').map(l => l.trim()).filter(l => l.length > 0); },
    activityImages() { if (!this.activityItem) return []; if (this.activityItem.images && Array.isArray(this.activityItem.images)) return this.activityItem.images; return this.activityItem.image ? [this.activityItem.image] : []; },
  },
  async created() {
  ...
  watch: {
    ...
    currentLang() { this.fetchActivity(); },
  },
  beforeUnmount() {
  ...
  methods: {
    ...
    nextImage() { if (this.activityImages.length === 0) return; this.currentImageIndex = (this.currentImageIndex + 1) % this.activityImages.length; },
